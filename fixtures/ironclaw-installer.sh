...
APP_NAME="ironclaw"
APP_VERSION="1.4.0"
if [ -n "${IRONCLAW_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$IRONCLAW_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${IRONCLAW_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$IRONCLAW_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/nearai/ironclaw/releases/download/ironclaw-v1.4.0"
elif [ -n "${IRONCLAW_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$IRONCLAW_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/nearai/ironclaw/releases/download/ironclaw-v1.4.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/nearai/ironclaw/releases/download/ironclaw-v1.4.0"
fi
if [ -n "${IRONCLAW_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$IRONCLAW_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${IRONCLAW_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$IRONCLAW_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${IRONCLAW_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$IRONCLAW_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${IRONCLAW_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${IRONCLAW_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${IRONCLAW_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"ironclaw","name":"ironclaw","owner":"nearai","release_type":"github"},"version":"1.4.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/ironclaw"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/ironclaw"
fi

usage() {
    # print help (this cat/EOF stuff is a "heredoc" string)
    cat <<EOF
ironclaw-installer.sh

The installer for ironclaw 1.4.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/nearai/ironclaw/releases/download/ironclaw-v1.4.0
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set IRONCLAW_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "ironclaw-aarch64-apple-darwin.tar.gz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="47118c9cf8ea97b067459460692e87d38ca2a8c6bb8f0e69c131278d351079af"
            _bins="ironclaw"
            _bins_js_array='"ironclaw"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-aarch64-apple-darwin-update"
            _updater_bin="ironclaw-aarch64-apple-darwin-update"
            ;;
        "ironclaw-aarch64-unknown-linux-gnu.tar.gz")
            _arch="aarch64-unknown-linux-gnu"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="d68285a0dd0232d111c45d5a0bbb41e7dc3f348cc1dac0969ac4fe97708cd046"
            _bins="ironclaw"
            _bins_js_array='"ironclaw"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-aarch64-unknown-linux-gnu-update"
            _updater_bin="ironclaw-aarch64-unknown-linux-gnu-update"
            ;;
        "ironclaw-aarch64-unknown-linux-musl.tar.gz")
            _arch="aarch64-unknown-linux-musl-static"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="f9340332472c523ab890c1cd66befb101e8a4eed3e49aacc48e14293fc7013f7"
            _bins="ironclaw"
            _bins_js_array='"ironclaw"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-aarch64-unknown-linux-musl-update"
            _updater_bin="ironclaw-aarch64-unknown-linux-musl-update"
            ;;
        "ironclaw-x86_64-apple-darwin.tar.gz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="638a212aaaab9d191612901ebd260653efe2b23e87e96eba45e7cc4bb052d964"
            _bins="ironclaw"
            _bins_js_array='"ironclaw"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-x86_64-apple-darwin-update"
            _updater_bin="ironclaw-x86_64-apple-darwin-update"
            ;;
        "ironclaw-x86_64-pc-windows-msvc.tar.gz")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="a2195a7788e108701151411557220c2e0cb841204f7a68b4bbd59de65e7d46cc"
            _bins="ironclaw.exe"
            _bins_js_array='"ironclaw.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-x86_64-pc-windows-msvc-update"
            _updater_bin="ironclaw-x86_64-pc-windows-msvc-update"
            ;;
        "ironclaw-x86_64-unknown-linux-gnu.tar.gz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="fa5d422dcaff510cdae13380304c40d4f46b5ac832d576b16131699cea5955ef"
            _bins="ironclaw"
            _bins_js_array='"ironclaw"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-x86_64-unknown-linux-gnu-update"
            _updater_bin="ironclaw-x86_64-unknown-linux-gnu-update"
            ;;
        "ironclaw-x86_64-unknown-linux-musl.tar.gz")
            _arch="x86_64-unknown-linux-musl-static"
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="409ac5624810b7a3a52454cd666dbc1906d7f65862d144e4878d68251d7a992e"
            _bins="ironclaw"
            _bins_js_array='"ironclaw"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            _updater_name="ironclaw-x86_64-unknown-linux-musl-update"
            _updater_bin="ironclaw-x86_64-unknown-linux-musl-update"
            ;;
        *)
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="ironclaw-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="ironclaw-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="ironclaw-x86_64-pc-windows-msvc.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="ironclaw-aarch64-unknown-linux-gnu.tar.gz"
            ...
            _archive="ironclaw-aarch64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="ironclaw-x86_64-unknown-linux-gnu.tar.gz"
            ...
            _archive="ironclaw-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${IRONCLAW_INSTALL_DIR:-}" ]; then
        _force_install_dir="$IRONCLAW_INSTALL_DIR"
